...
void draw() {
  ...
  //Display settings texts
  displaySimulatorSettings();
...
void keyPressed() {
  if(keyCode == DOWN){
    //Reset all infected except one
    for(int i = 0; i < totalPopulation; i++){
       //Cure everyone
       people[i].updateCured();
       people[i].changeColor(defaultColor);
    }
    
    //Pick a random number
    int unluckyPerson = int(random(totalPopulation));
    //Infect the unlucky person
    people[unluckyPerson].updateHealth("infected", "fatal", true);
    //Change color to red
    people[unluckyPerson].changeColorRGB(255,0,0);
  }
...
//Display simulator settings
void displaySimulatorSettings(){
  fill(255,255,255);
  textSize(16);
  text("Press DOWN ARROW KEY to RESET simulation", 500,25);
  
  text("ESC to EXIT", 950,25);
}
